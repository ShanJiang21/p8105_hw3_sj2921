...
* `aisle` is a character variable, which denotes the aisle for different products placement.
* `department_id` is a integer variable, for implying the department the products belong to, there are `r sum(depart)` distinct departments in total.
...
* `user_id` is used for marking each customers in the dataset, there are `r user_unique` customers signed up in the symstem.
```{r}
